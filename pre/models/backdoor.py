...
class GraphTrojanNet(nn.Module):
    ...
    def forward(self, input, thrd):
        ...
        # feat[-20:] = self.pattern(h).mean(dim=0).repeat(len(input),1)
        # feat[:,-40:] = self.pattern(h).mean(dim=0)
        # feat[:,(-10+1433):1433] = self.pattern(h).mean(dim=0)
        # feat[:,(-20+2866):2866] = self.pattern(h).mean(dim=0)
        
        # print('pattern',pattern.size())
        edge_weight = self.edge(h)
        # feat = GW(feat, thrd, self.device)
        edge_weight = GW(edge_weight, thrd, self.device)
        # edge_weight = torch.ones_like((edge_weight),device=self.device)

        # return feat, edge_weight
        # print('trojan_feat',feat.size())
        return feat, edge_weight
...
class Backdoor:
    ...
    #         # Plot the samples with the same label using a unique color
    #         # plt.scatter(label_features[:, 0], label_features[:, 1], label=f'Class {label}', cmap=color_map, alpha=0.5)
    #         plt.scatter(label_features[:, 0], label_features[:, 1], label='Clean', color='cornflowerblue', alpha=0.5)

    #     plt.scatter(last_30[:, 0], last_30[:, 1], color='red', label='Triggers', alpha=0.2)
    ...
    #     if len(features[0]) == self.args.hidden:
    #         # plt.savefig('./highlighted_embeddings.png')
    #         plt.savefig('./highlighted_embeddings_{}.png'.format(self.args.target_class))

    #     else:
    #         # plt.savefig('./highlighted_features.png')
    #         plt.savefig('./highlighted_features_{}.png'.format(self.args.target_class))
    def visualize_embeddings(self, features, labels, idx_attach):
        if features.is_cuda:
            features = features.cpu()
        ...
        # Plot the last 30 rows with one color
        # plt.scatter(last_30[:, 0], last_30[:, 1], color='red', label='Trigger')

        # Plot the rest with another color
        plt.scatter(rest[:, 0], rest[:, 1], color='cornflowerblue', label='Clean')
        plt.scatter(last_30[:, 0], last_30[:, 1], color='red', label='Triggers')
        # plt.scatter(features_2d[indices][:, 0], features_2d[indices][:, 1], color='green', label='Orignal Nodes')

        # plt.xlabel('Component 1')
        # plt.ylabel('Component 2')
        # plt.title('2D Visualization of Embeddings with Last 30 Nodes Highlighted')
    ...
    def con_loss(self, z1, z2, labels):
        ...
        # intra = f(self.simi(z1, z1))
        # inter = f(self.simi(z1, z2))     
        ...
        # loss = -torch.log(intra/denomitor).fill_diagonal_(0).mean()
        # here i think we should not choose representative ones as positive sample
        # to ensure the diversity of the triggers
...
# Sort z2_target based on these indices
    ...
    def fit(self, features, edge_index, edge_weight, labels, idx_train, idx_attach,idx_unlabeled,test=False):
        ...
        if test==True:
            ...
            # state_dict = torch.load('/home/zbz5349/defense_backdoor/DPGBA/kdd-backdooor/run_arxiv/model_weights.pth')
        ...
        for i in range(args.trojan_epochs):
            ...
            for j in range(self.args.inner):
                ...
                if i>0 and i%50==0 and j==self.args.inner-1:
                # if i%40==0 and j==self.args.inner-1:
                    self.visualize_embeddings(poison_x,labels,idx_attach )
                # if i==args.trojan_epochs-1 and j==self.args.inner-1:
                    # self.visualize_embeddings(torch.cat([poison_x[torch.cat([idx_train,idx_attach])],poison_x[-len(idx_attach)*self.args.trigger_size:]],dim=0),self.labels[torch.cat([idx_train,idx_attach])],idx_attach )
                    # self.visualize_embeddings(torch.cat([all_features[torch.cat([idx_train,idx_attach])],all_features[-len(idx_attach)*self.args.trigger_size:]],dim=0),self.labels[torch.cat([idx_train,idx_attach])],idx_attach )
                    
                    # self.visualize_embeddings_sne(torch.cat([all_features[torch.cat([idx_train,idx_attach])],all_features[-len(idx_attach)*self.args.trigger_size:]],dim=0),self.labels[torch.cat([idx_train,idx_attach])],idx_attach )
                    # self.visualize_embeddings_sne(all_features,labels,idx_attach )
            ...
            sim = self.con_loss_1(all_features[idx_outter],all_features[idx_train],self.labels[idx_train],output[idx_train])
            loss_targetclass = sim 
            - self.simi(all_features[idx_outter],all_features[idx_train]).fill_diagonal_(0).mean()
