...
from help_funcs import prune_unrelated_edge,prune_unrelated_edge_isolated, clu_prune_unrelated_edge, reconstruct_prune_unrelated_edge, str_reconstruct_prune_unrelated_edge
...
parser.add_argument('--dataset', type=str, default='Pubmed', 
                    help='Dataset',
                    choices=['Cora','Pubmed','Flickr','ogbn-arxiv','Citeseer'])
...
parser.add_argument('--inner', type=int,  default=2, help='Number of inner')
# backdoor setting
...
model.fit(data.x, train_edge_index, None, data.y, idx_train,idx_attach, unlabeled_idx, test=True)
poison_x, poison_edge_index, poison_edge_weights, poison_labels = model.get_poisoned()
l = len(poison_edge_index[0])-len(train_edge_index[0])
print('l',l)
if(args.defense_mode == 'prune'):
    ...
    poison_edge_index,poison_edge_weights = str_reconstruct_prune_unrelated_edge(args,poison_edge_index,poison_edge_weights,poison_x, data.x, data.edge_index, device,False)
    ...
    poison_edge_index,poison_edge_weights = clu_prune_unrelated_edge(args,poison_edge_index,poison_edge_weights,poison_x,device,False)
    bkd_tn_nodes = torch.cat([idx_train,idx_attach]).to(device)
print("precent of left attach nodes: {:.3f}"\
    # .format(len(set(bkd_tn_nodes.tolist()) & set(idx_attach.tolist()))/len(idx_attach)))
    .format((len(poison_edge_index[0])-len(train_edge_index[0]))/l))
